...
public abstract class AbstractTaskWorker {	
	...
	private final Executor getExecutor(Task task) {
		...
		return com.app.hrms.message.common.infra.TaskExecutor.IMMEDIATE_EXECUTOR;
	}
	
	private final Runnable getRunnable(final com.app.hrms.message.common.infra.Task task) {
		return new Runnable() {
			@Override
			public void run() {
				// schedule
